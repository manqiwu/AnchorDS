...
class SDSBridge(BaseLift3DSystem):
    ...
    class Config(BaseLift3DSystem.Config):
        ...
        guidance_init_type: str = "" # use this for sds at phase 1
        guidance_init: Optional[dict] = None
        prompt_processor_init_type: str = ""
        prompt_processor_init: Optional[dict] = None
    ...
    def configure(self) -> None:
        ...
        super().configure()
        if self.cfg.guidance_init_type: # use this for sds at phase 1
            self.guidance = threestudio.find(self.cfg.guidance_init_type)(self.cfg.guidance_init)
            self.prompt_processor = threestudio.find(self.cfg.prompt_processor_init_type)(
                self.cfg.prompt_processor_init
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.cfg.guidance_type == 'stable-diffusion-sds-bridge-guidance':
        ...
        else: # for anchorSD guidance
            if self.guidance.cfg.stage_two_start_step > 0:
                if self.true_global_step == self.guidance.cfg.stage_two_start_step:
                    threestudio.info(f"Moving to stage 2 at step {self.true_global_step}")
                    # reinitialize the guidance and prompt processor
                    # del existing ones to free memory
                    del self.guidance
                    del self.prompt_processor
                    del self.prompt_utils
                    # reinitialize
                    self.guidance = threestudio.find(self.cfg.guidance_type)(self.cfg.guidance)
                    self.prompt_processor = threestudio.find(self.cfg.prompt_processor_type)(
                        self.cfg.prompt_processor
