...
class StableDiffusionGuidance(BaseObject):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.cfg.use_sjc:
            ...
            guidance_eval_utils.update({"img_orig": rgb_BCHW_512})
    ...
    def guidance_eval(
        ...
        # get prev latent by denoising the latent_noisy using noise_pred_img; then get imgs_1orig_img_cond
        latents_1step_image_cond = []
        pred_1orig_image_cond = []
        for b in range(bs):
            step_output = self.scheduler.step(
                noise_pred_uncond[b : b + 1], t[b], latents_noisy[b : b + 1], eta=1
            )
