...
from gaussiansplatting.utils.mesh_utils import clean_mesh, decimate_mesh
from gaussiansplatting.scene.mesh import Mesh
...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int,
        ...
        self.active_sh_degree = 0 # spherical harmonics degree
        ...
        # SH (color)
        self._features_dc = torch.empty(0) # SH's main coefficients, representing color essentially
        self._features_rest = torch.empty(0) # SH's rest of coefficients, representing lighting
        # shape
        ...
        # additional
        self.max_radii2D = torch.empty(0) # max radii when projected to 2D
        self.xyz_gradient_accum = torch.empty(0) # to determine the need for densification
        self.denom = torch.empty(0) # number of updates for each point
        self.optimizer = None
    ...
    def densify_and_prune(
            self, max_grad, max_densify_percent=0.01, min_opacity=0.05, extent=4.0, max_screen_size=5
    ):
